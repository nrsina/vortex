...
pub fn list_scrollbar(
    ...
    // Thumb length is proportional to the fraction of the list that's visible,
    // clamped to at least one cell and at most the whole track. It depends only
    // on the list/viewport sizes — never on `selected` — so it stays constant
    // while scrolling and only resizes when the list does. `+ content_len / 2`
    ...
    // Thumb top: 0 at the first row, `travel` at the last, rounded to nearest.
    // `travel` is the number of cells the thumb can slide given its length.
    let travel = track_cells - thumb_len;
    let max_pos = content_len - 1;
    let sel = selected.min(max_pos);
    // `checked_div` yields `None` for a single-item list (max_pos == 0), where
    // the full-length thumb doesn't move anyway, so 0 is the right fallback.
